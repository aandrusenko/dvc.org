...
instead, like with regular project data tracked with `dvc add`. Use `dvc update`
to get the (latest) data from its import source.
...
value to `false` (or remove the field altogether).
